...
Next, you will need to copy in your unique application information, generated by the Ekko CLI in the previous section, to your client side code. Open up the `index.html` file, which contains the JavaScript code you'll need to edit. Replace `host` with your server endpoint, `jwt` with the Ekko client JWT, `appName` with the same appName you used to generate the JWT, and `uuid` can be whatever you choose, but is used to uniquely identify the client.
...
And finally, run your chat app by opening the `index.html` file in your browser window. You can open it up in multiple browser windows to test multiple clients. And voilà! You have a chat app using Ekko for realtime communication and in-transit message processing.
